...
public:
  ...
  OrderedSet(OrderedSet&& other) :
      amountOfNodes{std::move(other.amountOfNodes)},
      bst{std::move(other.bst)} {
  ...
  OrderedSet(InputIt first, InputIt last, Compare _comp = Compare()) :
      bst{std::move(_comp)} {
    insert(first, last);
  }
  OrderedSet(std::initializer_list<Key> initList, Compare _comp = Compare()) :
      bst{std::move(_comp)} {
...
bool operator==(const OrderedSet<Key, Compare>& lhs,
                const OrderedSet<Key, Compare>& rhs) {
  return std::equal(lhs.begin(), lhs.end(), rhs.begin(), rhs.end());
}

template <class Key, class Compare>
auto operator<=>(const OrderedSet<Key, Compare>& lhs,
                 const OrderedSet<Key, Compare>& rhs) {
